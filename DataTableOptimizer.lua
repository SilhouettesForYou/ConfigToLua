...
-- local test = require 'GachaponPackTable'
-- test.__sourcefile = 'GachaponPackTable.lua'
-- test.__name = 'GachaponPackTable'
-- local _localizedText = {}
-- local t, tableRef = ExportOptimizedDataset( test, _localizedText )
-- SaveDatasetToFile( t, true, tableRef )


function optimizer(src, name)
	local t = require (src)
	t.__sourcefile = src .. '.lua'
	t.__name = name
	local _localizedText = {}
	local _t, tableRef = ExportOptimizedDataset( t, _localizedText )
	SaveDatasetToFile( _t, true, tableRef )
end